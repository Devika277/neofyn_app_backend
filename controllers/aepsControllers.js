...
class AepsController {
    /**
     * Admin: Get all AePS transactions with Agent and Parent details
     * GET /api/aeps/admin/transactions
     */
    async adminGetAllTransactions(req, res) {
        try {
            const { status, type, from, to, search } = req.query;
            
            // Query with self-join to get Agent and Parent (Distributor) details
            let query = `
                SELECT 
                    t.*, 
                    u.first_name, u.last_name, u.phone, u.member_id,
                    p.first_name as parent_name, p.member_id as parent_member_id
            ...
            if (type) {
                query += ` AND t.txn_type = $${paramIndex}`;
                params.push(type);
                paramIndex++;
            }
            if (from) {
            ...
            if (search) {
                query += ` AND (u.first_name ILIKE $${paramIndex} OR u.phone ILIKE $${paramIndex} OR t.rrn ILIKE $${paramIndex})`;
                params.push(`%${search}%`);
                paramIndex++;
            }
            ...
            console.error('AepsController Error:', err);
