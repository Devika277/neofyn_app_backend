...
/**
 * Extract UTR from response data - checks ALL possible field names
 */
function extractUTR(data) {
  if (!data) return null;
  
  // Check all possible UTR field names (case-insensitive)
  const utrFields = [
    'utr', 'UTR', 'Utr',
    'utrNumber', 'utr_number', 'UTRNumber',
    'rrn', 'RRN', 'Rrn',
    'bankRefNo', 'bank_ref_no', 'BankRefNo',
    'referenceNumber', 'ReferenceNumber', 'reference_number',
    'txnRef', 'TxnRef', 'txn_ref',
    'refNo', 'RefNo', 'ref_no',
    'transactionReference', 'TransactionReference',
    'providerTxnId', 'provider_txn_id',
    'ackNo', 'AckNo', 'ack_no',
    'bankTxnId', 'BankTxnId', 'bank_txn_id'
  ];
  
  for (const field of utrFields) {
    if (data[field] && data[field] !== 'N/A' && data[field] !== 'NA' && data[field] !== 'null') {
      console.log(`[VimoPay DMT] ✅ UTR found in field "${field}": ${data[field]}`);
  ...
  if (data.data && typeof data.data === 'object') {
    const nested = extractUTR(data.data);
    if (nested) return nested;
  }
  
  if (data.result && typeof data.result === 'object') {
    const nested = extractUTR(data.result);
    if (nested) return nested;
  }
  
  console.log('[VimoPay DMT] ⚠️ No UTR found in response fields');
  return null;
}
...
async function sendDmtTransfer(params) {
  ...
  console.log('[VimoPay DMT] ========================================');
  console.log('[VimoPay DMT] Starting transfer');
  console.log('[VimoPay DMT] merchantRefId:', merchantRefId);
  console.log('[VimoPay DMT] amount:', amount);
  console.log('[VimoPay DMT] mode:', mode);
  console.log('[VimoPay DMT] ========================================');

  const token = await getAuthToken();

  // Get correct bank code
  ...
  if (!validBankCode || validBankCode === '001' || validBankCode === 'null' || validBankCode === 'undefined') {
    ...
    try {
      ...
      if (matchedBank) {
        ...
        console.log('[VimoPay DMT] ✅ Found bank code:', validBankCode, 'for bank:', matchedBank.bankName || matchedBank.name);
      } else {
...
//   }
  ...
  // };
const cleanIFSC = (ifsc) => {
    return ifsc ? ifsc.toUpperCase().trim() : '';
  };
  const cleanedName = cleanName(beneficiary.name);
  const cleanedIFSC = cleanIFSC(beneficiary.ifsc);
  ...
  console.log('[VimoPay DMT] 📤 SENDING PAYLOAD:');
  ...
  try {
    const response = await axios.post(
      ...
      console.log('[VimoPay DMT] 📥 RAW RESPONSE:');
    ...
    if (responseData.data) {
      try {
        decrypted = decrypt(responseData.data);
        console.log('[VimoPay DMT] 📥 DECRYPTED RESPONSE:');
        console.log(decrypted);
        
        try {
          data = JSON.parse(decrypted);
          console.log('[VimoPay DMT] 📥 PARSED DATA:');
          console.log(JSON.stringify(data, null, 2));
        } catch (e) {
          console.warn('[VimoPay DMT] Decrypted response is not JSON');
          data = { responseMessage: decrypted };
        }
      } catch (e) {
        console.error('[VimoPay DMT] Decryption failed:', e.message);
    ...
    // Handle failure
    if (!responseData || !responseData.successStatus) {
      console.error('[VimoPay DMT] ❌ API rejected request');
      
      let errorMessage = responseData?.message || 'VimoPay DMT API rejected request';
      
      if (Array.isArray(data)) {
        errorMessage = data.map(err => err.ErrorMessage || err.errorMessage).join('; ');
      } else if (data?.ErrorMessage) {
        errorMessage = data.ErrorMessage;
      } else if (data?.responseMessage) {
        errorMessage = data.responseMessage;
    ...
    if (!data && responseData.data) {
      try {
        const decrypted2 = decrypt(responseData.data);
        data = JSON.parse(decrypted2);
      } catch (e) {
        data = {};
      }
    ...
    if (code === '000' || code === '00' || code === 'SUCCESS') {
      ...
      const utr = extractUTR(data);
      console.log('[VimoPay DMT] ✅ SUCCESS - UTR:', utr);
      
      return {
        status: '000',
        providerRefId: data.txnId || data.transactionId || data.providerRefId || null,
        utr: utr,
        message: data.responseMessage || data.message || 'Success',
        rawData: data  // Include raw data for debugging
      };
    } else if (code === '004' || code === '04' || code === 'PENDING' || code === 'QUEUED') {
      console.log('[VimoPay DMT] ⏳ Transaction queued - awaiting webhook');
      return {
        status: '004',
        providerRefId: data.txnId || data.transactionId || null,
        utr: null,  // UTR will come via webhook
        message: 'Queued - awaiting callback',
        rawData: data
      };
    } else {
      const errorMsg = data.responseMessage || data.message || data.error || 'Transaction failed';
      console.warn('[VimoPay DMT] ❌ Failed with code:', code, 'Message:', errorMsg);
      return {
        status: code || '001',
        providerRefId: data?.txnId || null,
        utr: null,
    ...
    console.error('[VimoPay DMT] ❌ Network/System error:', error.message);
    if (error.response) {
      console.error('[VimoPay DMT] Error response:', JSON.stringify(error.response.data, null, 2));
    }
    throw new Error(`VimoPay DMT transfer failed: ${error.message}`);
...
async function getBankList() {
  ...
  let banks;
  try {
    const decrypted = decrypt(responseData.data);
    banks = JSON.parse(decrypted);
  } catch (e) {
    try {
      banks = JSON.parse(responseData.data);
    } catch (e2) {
