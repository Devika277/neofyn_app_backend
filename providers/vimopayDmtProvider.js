...
// ✅ ADD THIS - stateCoordinates for lat/long fallback
...
async function sendDmtTransfer(params) {
  ...
  // ✅ FIX: Use stateCoordinates for lat/long fallback
  const payload = {
  ...
  try {
    ...
    if (!responseData || !responseData.successStatus) {
      ...
      console.error('[VimoPay DMT] Transfer failed response:', {
        status: responseData?.responseCode || '001',
        providerRefId: data?.txnId || data?.transactionId || null,
        bankRefNo: data?.rrn || data?.txnId || null,
        utr: null,
    ...
    if (code === '000' || code === '00' || code === 'SUCCESS') {
      // ✅ FIXED: Extract UTR with proper fallback
      const utr = data.rrn || data.bankRefNo || data.txnId || null;
      ...
      const bankRef = data?.rrn || data?.bankRefNo || data?.txnId || null;
      const possibleUtr = data?.rrn || data?.bankRefNo || null;
      return {
        status: '004',
        providerRefId: data?.txnId || data?.transactionId || null,
        bankRefNo: bankRef,
    ...
    const enrichedError = new Error(`VimoPay DMT transfer failed: ${error.message}`);
    enrichedError.statusCode = error.response?.status?.toString() || 'error';
    enrichedError.rawResponse = error.response?.data || null;
    throw enrichedError;
  }
}
...
module.exports = { 
  ...
  getCityList, 
  getBankList,
  getAuthToken  // ✅ Export this for other modules
};